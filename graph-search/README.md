...
# 2. prim 알고리즘
prim 알고리즘은 시작 정점에서부터 출발하여 신장 트리 집합을 단계적으로 확장해나가는 방법이다. 

- prim 알고리즘 동작
    1. 시작 단계에서는 시작 정점만이 T(MST 집합)에 포함된다.
    2. T집합에 인접한 정점들 중에서 최소 간선으로 연결된 정점을 선택하여 트리를 확장한다. (즉, 가장 낮은 가중치를 먼저 선택한다.)
...
- prim의 시간복잡도는 주 반복문이 정점의 수 n만큼 반복하고, 내부 반복문이 n번 반복하므로 prim의 알고리즘은 O(n^2)의 복잡도를 가진다.
